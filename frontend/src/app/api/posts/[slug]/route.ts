...
export async function GET(_: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const query = db
    .from('blog_posts')
    .select('*, users!author_id(full_name, avatar_url), categories(id, name, slug)')

  const { data, error } = isUuid
    ? await query.eq('id', slug).single()
    : await query.eq('slug', slug).eq('is_published', true).single()
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const db = getSupabaseAdmin()
  const { data, error } = await db.from('blog_posts').update(body).eq('id', slug).select().single()
...
export async function DELETE(_: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const db = getSupabaseAdmin()
  const { error } = await db.from('blog_posts').delete().eq('id', slug)
  if (error) return NextResponse.json({ message: error.message }, { status: 500 })
